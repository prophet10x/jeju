#!/usr/bin/env bun
/**
 * Decentralized Network Startup Script
 * 
 * Starts all decentralized infrastructure services and validates they're healthy
 * before allowing applications to start. This ensures a fully permissionless,
 * resilient network with no single points of failure.
 * 
 * Services Started:
 * - IPFS (decentralized storage)
 * - Cache Service (Redis-compatible, runs in TEE)
 ...
 * 
 * Usage:
 *   bun run scripts/start-decentralized.ts
 *   bun run scripts/start-decentralized.ts --wait-only  # Just wait for services
 *   bun run scripts/start-decentralized.ts --check      # Health check only
 */
...
async function waitForServices(): Promise<ServiceHealth[]> {
  console.log('⏳ Waiting for decentralized services to be healthy...\n');
...
async function startDockerCompose(): Promise<void> {
  const composePath = join(process.cwd(), 'docker-compose.decentralized.yml');
  
  if (!existsSync(composePath)) {
    throw new Error(`docker-compose.decentralized.yml not found at ${composePath}`);
  }
...
async function printEnvVars(): Promise<void> {
  ...
  console.log('   export CQL_REQUIRED=true');
  console.log('');
}
...
async function main(): Promise<StartupResult> {
  ...
  console.log('\n');
  console.log('╔═══════════════════════════════════════════════════════════════════╗');
  console.log('║           JEJU DECENTRALIZED NETWORK STARTUP                       ║');
  ...
  if (!allHealthy) {
    ...
    console.log('   docker compose -f docker-compose.decentralized.yml logs');
    console.log('');
  ...
  console.log('╠═══════════════════════════════════════════════════════════════════╣');
  console.log('║  All decentralized services are healthy and running.              ║');
  console.log('║  No fallbacks. No single points of failure.                       ║');
  console.log('║                                                                   ║');
  console.log('║  Start apps: bun run dev                                          ║');
  console.log('║  Stop all:   docker compose -f docker-compose.decentralized.yml   ║');
  console.log('║              down && bun run localnet:stop                        ║');
  console.log('╚═══════════════════════════════════════════════════════════════════╝');
  console.log('\n');
...
export { main as startDecentralized, checkServiceHealth, waitForServices };
