...
async function main() {
  ...
  await test('RLAIF types are properly exported', async () => {
    const { RLAlgorithm, RLRunState } = await import('../apps/dws/src/rlaif/types');
    if (RLAlgorithm.GRPO !== 'grpo') throw new Error('RLAlgorithm.GRPO incorrect');
    if (RLRunState.CollectingRollouts !== 1) throw new Error('RLRunState incorrect');
  });

  await test('Coordinator can be imported', async () => {
    const { createRLAIFCoordinator } = await import('../apps/dws/src/rlaif/coordinator');
    if (typeof createRLAIFCoordinator !== 'function') throw new Error('Not a function');
  });

  await test('TrajectoryStore can be imported', async () => {
    const { createTrajectoryStore } = await import('../apps/dws/src/rlaif/trajectory-store');
    if (typeof createTrajectoryStore !== 'function') throw new Error('Not a function');
  });

  await test('RulerScorer can be imported', async () => {
    const { createRulerScorer } = await import('../apps/dws/src/rlaif/ruler-scorer');
    if (typeof createRulerScorer !== 'function') throw new Error('Not a function');
  });

  // Test Babylon Adapter
  console.log('\n5. Babylon Adapter Verification');
  await test('Babylon adapter exports are available', async () => {
    const exports = await import('../vendor/babylon/packages/training/src/compute/jeju-rlaif-adapter');
    if (!exports.BabylonJejuAdapter) throw new Error('BabylonJejuAdapter not exported');
    if (!exports.createBabylonJejuAdapter) throw new Error('createBabylonJejuAdapter not exported');
    if (!exports.trainWithJejuRLAIF) throw new Error('trainWithJejuRLAIF not exported');
  });

  await test('Babylon compute index exports adapter', async () => {
    const exports = await import('../vendor/babylon/packages/training/src/compute/index');
    if (!exports.BabylonJejuAdapter) throw new Error('BabylonJejuAdapter not in compute/index');
    if (!exports.trainWithJejuRLAIF) throw new Error('trainWithJejuRLAIF not in compute/index');
  });
  ...
  await test('Training command is exported', async () => {
    ...
    if (trainingCommand.name() !== 'training') throw new Error('Wrong command name');
  });
