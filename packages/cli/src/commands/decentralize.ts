...
function loadDeployment(network: NetworkType): Record<string, string> {
  ...
  const deployment = JSON.parse(readFileSync(deploymentPath, 'utf-8')) as DeploymentData;
...
async function transferOwnership(options: TransferOptions): Promise<void> {
  ...
  console.log('This command will transfer contract ownership to the GovernanceTimelock.');
  ...
  if (isMainnet) {
    ...
    const confirm1 = await prompt('Type "I UNDERSTAND THIS IS IRREVERSIBLE" to continue: ');
...
export const decentralizeCommand = new Command('decentralize')
  .description('Transfer contract ownership to GovernanceTimelock (IRREVERSIBLE)')
  ...
  .option('--contract <name>', 'Single contract to transfer (or all if omitted)')
  ...
  .action(async (options) => {
    ...
    if (options.contract && !CONTRACTS_TO_TRANSFER.includes(options.contract as TransferableContract)) {
      logger.error(`Invalid contract name. Must be one of: ${CONTRACTS_TO_TRANSFER.join(', ')}`);
      process.exit(1);
    }
