...
describe('LocalInferenceServer', () => {
  ...
  it('should list available models', async () => {
    ...
    expect(data.data.some((m: { id: string }) => m.id === 'local-fallback')).toBe(true);
  });
...
describe('ServicesOrchestrator', () => {
  ...
  describe('Mock JNS Service', () => {
    ...
    it('should return 404 for unknown names with availability info', async () => {
      const url = orchestrator.getServiceUrl('jns');
      const response = await fetch(`${url}/api/v1/resolve?name=nonexistent.jeju`);
    ...
    it('should return name pricing with length-based calculation', async () => {
      ...
      const response2 = await fetch(`${url}/api/v1/price?name=longname.jeju&years=1`);
    ...
    it('should list names for owner', async () => {
      const url = orchestrator.getServiceUrl('jns');
      const response = await fetch(`${url}/api/v1/names?owner=0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266`);
      expect(response.ok).toBe(true);
      const data = await response.json();
    ...
    it('should check name availability', async () => {
      ...
      const response2 = await fetch(`${url}/api/v1/available?name=randomname123.jeju`);
      const data2 = await response2.json();
    ...
    it('should register a new name', async () => {
      ...
      expect(response.ok).toBe(true);
      const data = await response.json() as { success: boolean; name: string; total: number };
      expect(data.success).toBe(true);
      expect(data.name).toBe(uniqueName);
      expect(data.total).toBeGreaterThan(0);
      
      // Verify registration
      const resolveResponse = await fetch(`${url}/api/v1/resolve?name=${uniqueName}`);
