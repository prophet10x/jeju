...
const SocialProviderSchema = z.enum(['google', 'apple', 'twitter', 'github', 'discord']);
...
export const PasskeyCredentialsArraySchema = z.array(PasskeyCredentialStorageSchema);
...
export function parseStoredSession(json: string): z.infer<typeof AuthSessionSchema> | null {
...
export function parseStoredPasskeys(json: string): z.infer<typeof PasskeyCredentialsArraySchema> {
  const parsed = JSON.parse(json);
...
export type StoredPasskeyCredential = z.infer<typeof PasskeyCredentialStorageSchema>;
