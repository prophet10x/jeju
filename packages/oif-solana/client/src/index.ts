...
// Program ID
export const OIF_PROGRAM_ID = new PublicKey('JejuOIF1111111111111111111111111111111111111');
...
export class OIFClient {
  ...
  async getOpenIntents(limit: number = 100): Promise<Intent[]> {
    // Fetch all intent accounts using getProgramAccounts
    const accounts = await this.connection.getProgramAccounts(this.programId, {
      filters: [
        { dataSize: 220 }, // Intent account size
      ],
    });
  ...
  async getActiveSolvers(limit: number = 100): Promise<Solver[]> {
    const accounts = await this.connection.getProgramAccounts(this.programId, {
      filters: [
        { dataSize: 150 }, // Solver account size (approximate)
      ],
    });
  ...
  async createIntentInstructions(
    ...
    // Convert destination token to bytes
  ...
  async fillIntentInstructions(
    ...
    // Create solver token account if needed
    ...
    instructions.push(
      new TransactionInstruction({
        keys: [
          ...
          { pubkey: config.authority, isSigner: false, isWritable: true }, // Fee account
          { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
          { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
          { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
        ],
  ...
  async cancelIntentInstructions(
    ...
    const discriminator = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03]); // cancel_intent
    ...
    return [
      new TransactionInstruction({
        keys: [
          ...
          { pubkey: solverPDA, isSigner: false, isWritable: true },
          { pubkey: owner, isSigner: false, isWritable: true }, // Stake amount
  ...
  private buildCreateIntentData(params: {
    ...
    // Discriminator for create_intent
  ...
  private buildRegisterSolverData(supportedChains: number[]): Buffer {
    ...
    // Discriminator for register_solver
  ...
  private addressToBytes(address: string): Uint8Array {
    // Handle both Solana (base58) and EVM (hex) addresses
