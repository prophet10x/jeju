...
const L1_RPC_PORT = parseInt(process.env.L1_RPC_PORT ?? '6545');
const L2_RPC_PORT = parseInt(process.env.L2_RPC_PORT ?? '6546');
const L2_WS_PORT = parseInt(process.env.L2_WS_PORT ?? '6547');
...
const L2_RPC_URL = process.env.L2_RPC_URL ?? process.env.JEJU_RPC_URL ?? `http://127.0.0.1:${L2_RPC_PORT}`;
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === '/send' && req.method === 'POST') {
        ...
        messages.push({
          ...
          from: envelope.from,
          to: envelope.to,
          content: typeof envelope.encryptedContent === 'string' 
      ...
      if (url.pathname === '/stats') {
        ...
        return Response.json({ nodeId: 'localnet-relay', totalMessagesRelayed: total });
      }
...
function startHubServer(): void {
  const castStore: Map<number, Array<{
    hash: string;
    fid: number;
    text: string;
    timestamp: number;
  ...
  hubServer = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === '/v1/userDataByFid') {
        ...
        return Response.json({
          messages: [
            { data: { fid: Number(fid), timestamp: Date.now(), userDataBody: { type: 'USER_DATA_TYPE_USERNAME', value: `user${fid}` } } },
            { data: { fid: Number(fid), timestamp: Date.now(), userDataBody: { type: 'USER_DATA_TYPE_DISPLAY', value: `User ${fid}` } } },
          ],
        });
      }
        ...
        return Response.json({
          messages: casts.map(c => ({
            ...
            data: {
              ...
              castAddBody: { text: c.text, embeds: [], mentions: [], mentionsPositions: [] },
            },
          })),
          nextPageToken: null,
        });
      ...
      if (url.pathname === '/v1/linksByFid' || url.pathname === '/v1/linksByTargetFid') {
        return Response.json({ messages: [], nextPageToken: null });
      }
...
beforeAll(async () => {
  ...
  if (localnetReady) {
    ...
    console.log('[Setup] Localnet not running - tests will use mock RPC responses');
  }
...
describe('Blockchain Connection', () => {
  ...
  test('can get account balances', async () => {
    ...
    const balance = await client.getBalance({ address: TEST_ACCOUNTS.deployer.address });
...
describe('Messaging Relay', () => {
  ...
  test('sends and receives messages', async () => {
    ...
    const sendResult = await sendResponse.json() as { success: boolean; messageId: string };
    expect(sendResult.success).toBe(true);
    expect(sendResult.messageId).toBe(messageId);
    
    // Receive
    const receiveResponse = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${recipient}`);
    expect(receiveResponse.ok).toBe(true);
    
    const messages = await receiveResponse.json() as { messages: Array<{ id: string }> };
  ...
  test('handles encrypted message flow', async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/bob-${bytesToHex(bobKeys.publicKey).slice(2, 12)}`);
    const data = await response.json() as { messages: Array<{ id: string; content: string }> };
    
    const received = data.messages.find(m => m.id === messageId);
    expect(received).toBeDefined();
    
    const decrypted = decrypt(new Uint8Array(Buffer.from(received!.content, 'base64')), sharedKey);
...
describe('Farcaster Hub', () => {
  ...
  test('fetches user data', async () => {
    const fid = 12345;
    const response = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/userDataByFid?fid=${fid}`);
    expect(response.ok).toBe(true);
  ...
  test('submits and retrieves cast', async () => {
    ...
    // Submit
    const submitResponse = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/submitMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
          ...
          castAddBody: { text: castText, embeds: [], mentions: [], mentionsPositions: [] },
        },
      }),
    });
    ...
    // Retrieve
    const castsResponse = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/castsByFid?fid=${fid}`);
    expect(castsResponse.ok).toBe(true);
    
    const casts = await castsResponse.json() as { messages: Array<{ data: { castAddBody: { text: string } } }> };
    expect(casts.messages.some(m => m.data.castAddBody.text === castText)).toBe(true);
  });
  
  test('submits reaction', async () => {
    const response = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/submitMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
...
describe('MLS Groups', () => {
  test('creates group with members', () => {
    const group = {
      ...
      members: [TEST_ACCOUNTS.alice.address, TEST_ACCOUNTS.bob.address, TEST_ACCOUNTS.charlie.address],
  ...
  test('sends messages to group', async () => {
    ...
    const sendToGroup = (sender: string, content: string) => {
      messages.push(JSON.stringify({ sender, content, groupId, timestamp: Date.now() }));
    };
...
describe('Direct Casts', () => {
  test('sends encrypted DM', async () => {
    ...
    // Verify decryption
    const decrypted = decrypt(new Uint8Array(Buffer.from(encrypted, 'base64')), key);
  ...
  test('retrieves conversation messages', async () => {
    ...
    // Retrieve
    const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${conversationId}`);
...
describe('End-to-End Flows', () => {
  test('public Farcaster cast followed by private DM', async () => {
    ...
    // 1. Post public cast
    const castResponse = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/submitMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
    ...
    const messages = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/fid:${userFid}`);
  ...
  test('group creation and messaging', async () => {
    const groupId = `group-${crypto.randomUUID()}`;
    const members = [TEST_ACCOUNTS.alice.address, TEST_ACCOUNTS.bob.address, TEST_ACCOUNTS.charlie.address];
    
    // 1. Create group (metadata)
    const group = { id: groupId, name: 'E2E Test Group', members, createdAt: Date.now() };
    ...
    const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${groupId}`);
    const data = await response.json() as { count: number };
  ...
  test('cross-platform: Farcaster identity + XMTP messaging', async () => {
    ...
    const linkResponse = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/submitMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
    ...
    await fetch(`http://127.0.0.1:${HUB_PORT}/v1/submitMessage`, {
      ...
      body: JSON.stringify({
        data: {
          type: 'CAST_ADD',
          fid: userFid,
          castAddBody: { text: 'Cross-platform messaging works', embeds: [], mentions: [], mentionsPositions: [] },
        },
      }),
    });
    ...
    const xmtpMessages = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${userAddress}`);
    const xmtpData = await xmtpMessages.json() as { messages: Array<{ id: string }> };
    expect(xmtpData.messages.some(m => m.id === xmtpMsgId)).toBe(true);
    
    const farcasterCasts = await fetch(`http://127.0.0.1:${HUB_PORT}/v1/castsByFid?fid=${userFid}`);
    const castData = await farcasterCasts.json() as { messages: Array<{ data: { castAddBody: { text: string } } }> };
    expect(castData.messages.some(m => m.data.castAddBody.text.includes('Cross-platform'))).toBe(true);
  });
});
...
describe('Performance', () => {
  test('handles 50 concurrent messages', async () => {
    ...
    // Verify all received
    const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${recipient}`);
    const data = await response.json() as { count: number };
