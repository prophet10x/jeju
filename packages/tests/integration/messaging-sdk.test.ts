...
const messageStore: Map<string, Array<{
  ...
  from: string;
  to: string;
  encryptedContent: { ciphertext: string; nonce: string; ephemeralPublicKey: string };
  timestamp: number;
}>> = new Map();
          ...
          messages: [
            {
              data: {
                fid: Number(fid),
                timestamp: Date.now(),
                userDataBody: { type: 'USER_DATA_TYPE_USERNAME', value: `user${fid}` },
              },
            },
            {
              data: {
                fid: Number(fid),
                timestamp: Date.now(),
                userDataBody: { type: 'USER_DATA_TYPE_DISPLAY', value: `User ${fid}` },
              },
            },
          ],
        });
...
function serializeEncryptedMessage(encrypted: { ciphertext: Uint8Array; nonce: Uint8Array; ephemeralPublicKey: Uint8Array }): {
  ciphertext: string;
  nonce: string;
...
describe('Messaging SDK', () => {
  ...
  describe('Relay Server', () => {
    ...
    test('accepts and stores messages', async () => {
      ...
      const result = await response.json() as { success: boolean; messageId: string; cid: string };
    ...
    test('retrieves pending messages for recipient', async () => {
      ...
      const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${bobAddress}`);
      expect(response.ok).toBe(true);
  ...
  describe('E2E Flow', () => {
    test('complete message flow: encrypt -> send -> receive', async () => {
      ...
      const { messages } = await fetchResponse.json() as { messages: Array<{ id: string; from: string }> };
    ...
    test('multiple messages between users', async () => {
      ...
      // Fetch all
      const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${bobAddress}`);
      const { messages, count } = await response.json() as { messages: Array<{ id: string; from: string }>; count: number };
...
describe('Farcaster SDK', () => {
  ...
  describe('Hub Client', () => {
    test('fetches hub info', async () => {
      ...
      const info = await response.json() as { version: string; isSyncing: boolean; nickname: string };
    ...
    test('fetches user data by FID', async () => {
      const fid = 12345;
      const response = await fetch(`http://127.0.0.1:${MOCK_HUB_PORT}/v1/userDataByFid?fid=${fid}`);
      expect(response.ok).toBe(true);
      
      const data = await response.json() as { messages: Array<{ data: { fid: number; userDataBody: { value: string } } }> };
    ...
    test('fetches casts by FID', async () => {
      const fid = 54321;
      const response = await fetch(`http://127.0.0.1:${MOCK_HUB_PORT}/v1/castsByFid?fid=${fid}`);
      expect(response.ok).toBe(true);
...
describe('MLS Groups', () => {
  ...
  test('tracks message history in group', () => {
    ...
    const messages: Array<{ id: string; groupId: string; sender: string; content: string; timestamp: number }> = [];
  ...
  test('handles member management', () => {
    ...
    expect(members.has('0x70997970C51812dc3A010C7d01b50e0d17dc79C8')).toBe(false);
  });
});
...
describe('Combined Messaging Flow', () => {
  ...
  test('Farcaster cast followed by XMTP DM', async () => {
    ...
    const castResponse = await fetch(`http://127.0.0.1:${MOCK_HUB_PORT}/v1/submitMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
    ...
    // 3. Alice receives DM
    const messagesResponse = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/0xAliceAddress`);
  ...
  test('Group creation after 1:1 DM', async () => {
    ...
    const groupMsgResponse = await fetch(`http://127.0.0.1:${RELAY_PORT}/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: crypto.randomUUID(),
    ...
    const groupMsgs = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/${group.id}`);
...
describe('Error Handling', () => {
  ...
  test('handles missing recipient messages', async () => {
    const response = await fetch(`http://127.0.0.1:${RELAY_PORT}/messages/0xNonexistent`);
    expect(response.ok).toBe(true);
