...
import { getContracts, DEFAULT_RPC, ZERO_ADDRESS, CHAIN_IDS } from './config.js';
...
export class OAuth3JNSService {
  ...
  constructor(config: OAuth3JNSConfig = {}) {
    ...
    this.defaultResolverAddress = config.resolverAddress || contracts.jnsResolver;
  }
  ...
  async resolveApp(appName: string): Promise<OAuth3AppJNS | null> {
    ...
    const [council, redirectUris, authEndpoint, callbackEndpoint, policyUri, termsUri, avatar] = await Promise.all([
    ...
    return {
      ...
      callbackEndpoint: callbackEndpoint || (endpoint ? `${endpoint}/callback` : ''),
      metadata: { logoUri: avatar || '', policyUri: policyUri || '', termsUri: termsUri || '' },
    };
  }
  ...
  async resolveIdentity(identityName: string): Promise<IdentityJNS | null> {
    ...
    const [smartAccount, publicKey, linkedProviders, avatar] = await Promise.all([
  ...
  async resolveTEENode(nodeName: string): Promise<TEENodeJNS | null> {
    ...
    const [endpoint, publicKey, attestationType, supportedProviders, stake, active] = await Promise.all([
  ...
  async getRecords(name: string): Promise<JNSRecords> {
    ...
    const [address, avatar, url, description, oauth3Endpoint, a2aEndpoint] = await Promise.all([
      this.client.readContract({ address: resolverAddress, abi: JNS_RESOLVER_ABI, functionName: 'addr', args: [node] }).catch(() => null),
      this.getText(resolverAddress, node, 'avatar'),
  ...
  private async getText(resolverAddress: Address, node: Hex, key: string): Promise<string | null> {
    return this.client.readContract({
      address: resolverAddress,
      abi: JNS_RESOLVER_ABI,
      functionName: 'text',
      ...
      if (error.message.includes('reverted') || error.message.includes('empty')) {
        return null;
      }
      throw error;
    });
  ...
  getRegistryAddress(): Address { return this.registryAddress; }
  getDefaultResolverAddress(): Address { return this.defaultResolverAddress; }
}

let instance: OAuth3JNSService | null = null;

export function createOAuth3JNSService(config: OAuth3JNSConfig = {}): OAuth3JNSService {
  if (!instance) instance = new OAuth3JNSService(config);
  return instance;
}

export function resetOAuth3JNSService(): void { instance = null; }
