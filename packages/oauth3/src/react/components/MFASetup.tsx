...
import { MFAMethod } from '../../mfa/index.js';
...
export function MFASetup({
  ...
  const [view, setView] = useState<'main' | 'totp' | 'passkey' | 'backup'>('main');
  const [totpSetup, setTotpSetup] = useState<{ secret: string; uri: string; qrCode: string } | null>(null);
  const [totpCode, setTotpCode] = useState('');
  const [passkeyName, setPasskeyName] = useState('');
  const [passkeys, setPasskeys] = useState<Array<{ id: string; deviceName: string; createdAt: number }>>([]);
  ...
  const handleSetupPasskey = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    setLocalError(null);
  ...
  if (view === 'totp' && totpSetup) {
    return (
      <div style={containerStyle} className={className}>
        <button style={backButtonStyle} onClick={() => { setView('main'); setTotpSetup(null); }}>
          ← Back
        </button>
        ...
        <form onSubmit={handleVerifyTOTP}>
          <input
            ...
            required
          />
          <button type="submit" style={{ ...buttonStyle, width: '100%' }} disabled={isLoading}>
  ...
  if (view === 'passkey') {
    return (
      <div style={containerStyle} className={className}>
        ...
        <p style={subtitleStyle}>
          Use your device's biometrics or security key for passwordless authentication.
        </p>
        ...
        {passkeys.length > 0 && (
          <div style={{ marginBottom: '24px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 600, marginBottom: '12px' }}>Your Passkeys</h4>
            {passkeys.map(pk => (
              <div key={pk.id} style={{
                display: 'flex',
  ...
  if (view === 'backup') {
    return (
      <div style={containerStyle} className={className}>
        <button style={backButtonStyle} onClick={() => { setView('main'); setBackupCodes([]); }}>
          ← Back
        </button>
        ...
        <div style={{
          ...
          <strong>⚠️ Important:</strong> These codes will only be shown once. Save them now!
        </div>
        ...
        <button
          onClick={() => { setView('main'); setBackupCodes([]); }}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {mfaStatus?.totpEnabled ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <button
                  onClick={e => { e.stopPropagation(); handleDisableTOTP(); }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              {(mfaStatus?.passkeyCount ?? 0) > 0 && (
                <span style={{ color: '#22C55E' }}>
                  {mfaStatus?.passkeyCount} key{(mfaStatus?.passkeyCount ?? 0) > 1 ? 's' : ''}
                </span>
              )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: '#4F46E5' }}>
                {(mfaStatus?.backupCodesRemaining ?? 0) > 0 ? 'Regenerate →' : 'Generate →'}
              </span>
            </div>
          </div>
        </div>
