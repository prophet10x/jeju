...
interface PublicKeyCredentialCreationOptions {
  ...
  timeout?: number;
  excludeCredentials?: Array<{ type: 'public-key'; id: ArrayBuffer; transports?: AuthenticatorTransport[] }>;
  authenticatorSelection?: AuthenticatorSelectionCriteria;
  attestation?: AttestationConveyancePreference;
}
...
interface PublicKeyCredentialRequestOptions {
  ...
  timeout?: number;
  rpId?: string;
  allowCredentials?: Array<{ type: 'public-key'; id: ArrayBuffer; transports?: AuthenticatorTransport[] }>;
  userVerification?: UserVerificationRequirement;
}
...
export class PasskeyManager {
   ...
   */
  async generateRegistrationOptions(options: PasskeyRegistrationOptions): Promise<{
    challengeId: string;
  ...
  async verifyRegistration(
    ...
    const attestationObject = new Uint8Array(response.response.attestationObject);
    ...
    const credential: PasskeyCredential = {
      ...
      createdAt: Date.now(),
      lastUsedAt: Date.now(),
      transports: response.authenticatorAttachment === 'platform' ? ['internal'] : undefined,
    };
   ...
   */
  async generateAuthenticationOptions(options: PasskeyAuthenticationOptions = {}): Promise<{
    challengeId: string;
    ...
    let allowCredentials: Array<{ type: 'public-key'; id: ArrayBuffer; transports?: AuthenticatorTransport[] }> | undefined;
   ...
   */
  updateCredentialName(userId: string, credentialId: string, deviceName: string): boolean {
    const userCredentials = this.credentials.get(userId);
    if (!userCredentials) return false;
  ...
  private getSignCount(authenticatorData: Uint8Array): number {
    ...
    const view = new DataView(authenticatorData.buffer, authenticatorData.byteOffset + 33, 4);
...
export function createPasskeyManager(config?: { rpId?: string; rpName?: string }): PasskeyManager {
