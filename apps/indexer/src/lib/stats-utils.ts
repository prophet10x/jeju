...
import {
  ComputeProvider, StorageProvider, ContainerImage, CrossServiceRequest,
  ComputeRental, StorageDeal, RegisteredAgent, OracleFeed, OracleOperator,
...
export async function getOracleStats(dataSource: DataSource): Promise<OracleStats> {
  if (!dataSource) {
    throw new Error('DataSource is required');
  }
  ...
  const [totalFeeds, activeFeeds, operators, totalReports, disputedReports, totalDisputes, openDisputes, totalSubscriptions, activeSubscriptions] = await Promise.all([
    ...
    dataSource.getRepository(OracleReport).count({ where: { isDisputed: true } }),
    dataSource.getRepository(OracleDispute).count(),
    dataSource.getRepository(OracleDispute).count({ where: { status: 'OPEN' as never } }),
    dataSource.getRepository(OracleSubscription).count(),
    dataSource.getRepository(OracleSubscription).count({ where: { isActive: true } }),
  ]);
...
export async function getNetworkStats(dataSource: DataSource): Promise<NetworkStats> {
  if (!dataSource) {
    throw new Error('DataSource is required');
  }

  const { Block, Transaction, Account, Contract, RegisteredAgent, NodeStake } = await import('../model');
  ...
  const latestBlock = await dataSource.getRepository(Block).createQueryBuilder('b').orderBy('b.number', 'DESC').limit(1).getOne();
