...
export async function getContainerDetail(
  dataSource: DataSource,
  cid: string
): Promise<{ container: ContainerDetail; compatibleProviders: CompatibleProvider[] }> {
  if (!dataSource) {
    throw new Error('DataSource is required');
  }
  ...
  return {
    container: {
      ...
      verifiedBy: container.verifiedBy?.agentId ? container.verifiedBy.agentId.toString() : null,
