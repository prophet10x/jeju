...
import { createCrucibleRuntime, checkDWSHealth, checkDWSInferenceAvailable } from '../src/sdk/eliza-runtime';
...
async function main() {
  ...
  if (result1.actionsExecuted.length > 0) {
    ...
    for (const action of result1.actionsExecuted) {
      console.log(`    - ${action.name}: ${action.success ? 'SUCCESS' : 'FAILED'}`);
    }
  } else {
  ...
  config.goals = [
    {
      id: 'goal-2',
      description: 'Check if any agents are available for collaboration based on previous network scan',
      status: 'active',
      priority: 'medium',
    },
  ];
  ...
  console.log(`Tick 1: ${result1.success ? 'SUCCESS' : 'NEEDS REVIEW'} (${result1.actionsExecuted.length} actions)`);
  console.log(`Tick 2: ${result2.success ? 'SUCCESS' : 'NEEDS REVIEW'} (${result2.actionsExecuted.length} actions)`);
  console.log(`Memory persistence: ${memories.length >= 2 ? 'WORKING' : 'NEEDS REVIEW'}`);
  console.log(`Total autonomous iterations: ${result1.iterations + result2.iterations}`);
