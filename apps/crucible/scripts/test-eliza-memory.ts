...
import { createCrucibleRuntime, type RuntimeMessage, checkDWSHealth, checkDWSInferenceAvailable } from '../src/sdk/eliza-runtime';
...
async function main() {
  ...
  if (!dwsOk) {
    console.error('DWS not available. Start DWS first: cd apps/dws && bun run dev');
    process.exit(1);
  }
  ...
  if (!inference.available) {
    console.error('No inference nodes. Start inference: cd apps/dws && bun run inference');
    process.exit(1);
  }
  ...
  if (actions.length > 0) {
    console.log(`   Sample actions: ${actions.slice(0, 5).map(a => a.name).join(', ')}`);
  }
  ...
  console.log(`"${response1.text.substring(0, 200)}${response1.text.length > 200 ? '...' : ''}"`);

  // Verify it's not a canned response
  const isCanned = response1.text.includes('Hello! How can I help') || 
  ...
  console.log('Setting context: "My name is Alex and I work on the Jeju SDK. Remember that."');

  const msg2: RuntimeMessage = {
    id: crypto.randomUUID(),
    userId: USER_ID,
    roomId: ROOM_ID,
    content: { text: 'My name is Alex and I work on the Jeju SDK. Remember that.', source: 'test' },
    createdAt: Date.now(),
  };
  ...
  const mentionsPepe = response3.text.toLowerCase().includes('alex') || response3.text.toLowerCase().includes('jeju');
  if (mentionsPepe) {
    console.log('✅ Agent correctly recalled context from memory (Alex or Jeju SDK)');
  } else {
    console.log('⚠️  Agent did not recall context (memory may not be working correctly)');
  }
  ...
  if (response4.action) {
    ...
    console.log('⚠️  No action triggered (agent may respond conversationally first)');
  }
