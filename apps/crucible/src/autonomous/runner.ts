...
import { createCrucibleRuntime, type CrucibleAgentRuntime } from '../sdk/eliza-runtime';
...
export class AutonomousAgentRunner {
    ...
    if (this.config.enableBuiltinCharacters) {
      ...
      for (const characterId of characterIds) {
        if (this.agents.size >= this.config.maxConcurrentAgents) {
          log.warn('Max concurrent agents reached', { max: this.config.maxConcurrentAgents });
          break;
        }
  ...
  private async runTickCycle(): Promise<void> {
    ...
    const batchSize = Math.min(dueAgents.length, this.config.maxConcurrentAgents);
   ...
   */
  private async executeAgentTick(agentId: string): Promise<AutonomousTickResult> {
    const agent = this.agents.get(agentId);
    if (!agent) {
      return { success: false, actionsExecuted: [], iterations: 0, duration: 0, error: 'Agent not found' };
    }
    ...
    try {
      ...
      if (execution.errors >= 3) {
        const backoff = Math.min(execution.errors * config.tickIntervalMs, 300_000); // Max 5 min
