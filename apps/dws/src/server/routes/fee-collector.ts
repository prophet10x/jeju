...
let pendingFees: Map<string, FeeDeposit[]> = new Map();
let feeStats: FeeStats = {
  totalCollected: 0n,
  totalDeposited: 0n,
  pendingDeposit: 0n,
  bySource: {},
  ...
  for (const [daoId, fees] of pendingFees.entries()) {
    ...
    for (const [source, amount] of bySource.entries()) {
      ...
      try {
        ...
        console.log(`[FeeCollector] Deposited ${amount} for ${source} to DAO ${daoId.slice(0, 10)}`);
      } catch (err) {
...
export function createFeeCollectorRouter(): Hono {
  ...
  router.post('/record', async (c) => {
    const body = await c.req.json() as { daoId: string; source: string; amount: string };
