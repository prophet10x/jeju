...
app.post('/judge', async (c) => {
  ...
  const psycheClient = createPsycheClient({
    ...
    llmJudgeUrl: body.llmJudgeUrl ?? process.env.LLM_JUDGE_URL ?? 'http://localhost:9001',
    llmJudgeModel: body.llmJudgeModel ?? process.env.LLM_JUDGE_MODEL ?? 'default',
  });
...
app.get('/bridge/runs/:runId', async (c) => {
  ...
  const bridge = createCrossChainBridge({
    ...
    solanaRpcUrl: process.env.SOLANA_RPC_URL ?? 'http://localhost:8899',
    bridgeContractAddress: (process.env.BRIDGE_ADDRESS ?? '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512') as Address,
  });
...
app.post('/bridge/merkle/root', async (c) => {
  ...
  const bridge = createCrossChainBridge({
    ...
    bridgeContractAddress: '0x0000000000000000000000000000000000000000' as Address,
  });
