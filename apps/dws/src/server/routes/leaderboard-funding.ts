...
async function fetchLeaderboard(limit: number = 50): Promise<LeaderboardEntry[]> {
  ...
  try {
    const response = await fetch(`${leaderboardUrl}/api/leaderboard?limit=${limit}`);
    if (!response.ok) {
...
async function syncLeaderboardToFunding(
  ...
  for (const entry of leaderboard) {
    try {
      ...
      }) as [Hex, Address, bigint, number, string, bigint, bigint, bigint, boolean];
      ...
      synced++;
      console.log(`[LeaderboardFunding] Set weight ${weight} for ${entry.username}`);
    } catch (err) {
