...
import { createWalletClient, createPublicClient, http, keccak256, parseAbi, type Address, type Hex } from 'viem';
...
async function fetchNpmDependencies(packageJson: Record<string, Record<string, string>>): Promise<Record<string, string>> {
  return {
...
async function fetchNpmTransitiveDeps(packageName: string): Promise<string[]> {
  try {
    ...
    const data = await response.json() as { 'dist-tags'?: { latest?: string }; versions?: Record<string, { dependencies?: Record<string, string> }> };
...
async function fetchPyPIDependencies(requirementsTxt: string): Promise<Record<string, string>> {
...
async function fetchCargoDependencies(cargoToml: string): Promise<Record<string, string>> {
  const deps: Record<string, string> = {};
...
async function scanRepository(
  ...
  const addDep = (name: string, version: string, registryType: string, depth: number) => {
      ...
      try {
        ...
        for (const [name, version] of Object.entries(directDeps)) {
          ...
          if (maxDepth > 0) {
            ...
            for (const transDep of transitive.slice(0, 20)) { // Limit transitive
  ...
  if (registryTypes.includes('pypi')) {
    const requirements = await fetchRepoFile(owner, repo, 'requirements.txt', token);
    if (requirements) {
...
export function createDependencyScannerRouter(): Hono {
  ...
  const distributorAddress = process.env.DEEP_FUNDING_DISTRIBUTOR_ADDRESS as Address;
  ...
  router.post('/scan', async (c) => {
    ...
    const { daoId, repoOwner, repoName, registryTypes, maxDepth, autoRegister } = body;
