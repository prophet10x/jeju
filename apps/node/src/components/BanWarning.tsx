...
export function BanWarning() {
  ...
  const canAppeal = banStatus.appeal_deadline && banStatus.appeal_deadline > Date.now() / 1000;
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <AlertTriangle 
          size={24} 
          className={isPermanent ? 'text-red-400' : isOnNotice ? 'text-yellow-400' : 'text-orange-400'} 
        />
        
        <div className="flex-1">
          <h3 className={`font-semibold ${
            isPermanent ? 'text-red-300' : isOnNotice ? 'text-yellow-300' : 'text-orange-300'
          }`}>
            {isPermanent 
          ...
          {canAppeal && banStatus.appeal_deadline && (
            <div className="flex items-center gap-2 mt-2 text-sm text-volcanic-400">
              <Clock size={14} />
              <span>
                Appeal deadline: {new Date(banStatus.appeal_deadline * 1000).toLocaleDateString()}
              </span>
            </div>
          )}
