...
async function fetchAgents(query?: { type?: AgentType; status?: AgentStatus; owner?: string }): Promise<Agent[]> {
...
async function updateAgent(agentId: string, data: Partial<Agent>): Promise<boolean> {
  const dwsUrl = getDwsUrl();
  const res = await fetch(`${dwsUrl}/api/agents/${agentId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export function useAgents(query?: { type?: AgentType; status?: AgentStatus; owner?: string }) {
