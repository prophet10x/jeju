...
export default function MultiTokenBalanceDisplay() {
  ...
  const displayBalances = useMemo(() => {
    const next = { ...balances }
    if (smartJejuBalance !== undefined) {
      next.JEJU = smartJejuBalance as bigint
    }
    return next
  }, [balances, smartJejuBalance])

  const totalUSD = tokens.reduce((sum, token) => {
    const balance = displayBalances[token.symbol]
    ...
    <div className="card" style={{ marginBottom: '1.5rem' }}>
      ...
      >
        {sortedTokens.map((token) => {
          const balance = displayBalances[token.symbol] ?? 0n
