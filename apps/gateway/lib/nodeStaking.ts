...
export const NODE_STAKING_MANAGER_ABI = [
  ...
  },
  {
    ...
    name: 'SlashProposed',
    inputs: [
      { name: 'slashId', type: 'bytes32', indexed: true },
      { name: 'nodeId', type: 'bytes32', indexed: true },
      { name: 'slashPercentageBPS', type: 'uint256', indexed: false },
      { name: 'reason', type: 'string', indexed: false },
    ],
  },
] as const
...
import {
  getNodeStakingReadAddresses as resolveNodeStakingReadAddresses,
...
export function getNodeStakingReadAddresses(): Address[] {
  return resolveNodeStakingReadAddresses()
}
